    ...
    <div class="row"> @foreach($users as $user) <div class="col-sm-12 col-md-3 p-3 p-md-5">
        <div class="card">
          <div class="card-body bg-dark rounded">
            ...
            <br>
            <center><form action="{{ route('users.destroy', $user->id) }}" method="POST"> @csrf @method('DELETE') <input onclick="return confirm('Hapus USer ? ' + 'elemento?');" type="submit" value="Delete"class="btn col-4 btn btn-danger btn-block" ></center>
            </form>
          </div>
        </div>
      </div> @endforeach </div>
